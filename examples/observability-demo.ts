...
async function main() {
  ...
  await CorrelationContext.run({ correlationId: 'demo-trace-12345', tenantId: 'T1' }, async () => {
    logger.info('Starting transaction processing wave');

    // 4. Trace database calls using database instrumenter
    const dbInstrumenter = new DatabaseInstrumenter({ dbSystem: 'redis', dbName: 'state-store' });
    const driverList = await dbInstrumenter.traceCall('getNearbyDrivers', async () => {
      // Simulate database fetch delay
      await new Promise(resolve => setTimeout(resolve, 50));
      return ['driver-1', 'driver-2'];
    }, { radiusMeters: 5000 });

    logger.info(`Successfully fetched ${driverList.length} nearby candidates`);

    // 5. Trace event publish on the bus
    await EventInstrumenter.tracePublish('session.assigned', async () => {
