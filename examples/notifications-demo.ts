...
async function main() {
  ...
  notificationService.getTemplates().register({
    ...
    bodyTemplate:
      "Hi {{driverName}}, session {{sessionId}} is open for grabs near {{pickup}}!",
  });
  ...
  await targeting.registerToken(
    "T1",
    "driver-101",
    "ios-device-token-abc",
    "ios"
  );
  await targeting.registerToken(
    "T1",
    "driver-102",
    "android-device-token-xyz",
    "android"
  );
  ...
  console.log(
    `Driver 101 alert status: ${
      resultApns.success ? "Delivered" : "Failed"
    } via ${resultApns.providerName}`
  );
  ...
  console.log(
    `Driver 102 alert status: ${
      resultFcm.success ? "Delivered" : "Failed"
    } via ${resultFcm.providerName}`
  );
  ...
  setTimeout(async () => {
    const processed = await notificationService
      ...
      .processPendingJobs(async (tenant, payload) => {
        console.log(
          `Scheduler dispatched alert: ${payload.title} to recipient ${payload.recipientId}`
        );
      });
