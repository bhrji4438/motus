...
if (hasNpmToken) {
  console.log('NPM_TOKEN environment variable is set. Running changeset publish...');
  execSync('npx changeset publish', { stdio: 'inherit' });
} else {
  console.log('NPM_TOKEN environment variable is NOT set. Running dry-run publish for all public packages...');
  ...
  for (const pkg of packages) {
    ...
    try {
      ...
      console.error(`\x1b[31m[ERROR]\x1b[0m Failed dry-run publish for ${pkgJson.name}:`, err.message);
      failed = true;
    }
  }
