...
const makeDriver = (
  overrides?: Partial<Driver & { vehicleType: string }>
...
describe("RedisDriverRepository (integration)", () => {
  ...
  it("rejects cross-tenant access", async () => {
    ...
    await ctx.client.hset(
      KeyFactory.driverHash("tnt_OTHER", driver.id),
      fields
    );

    // Try to fetch driver from a different tenant
    await expect(repo.get("tnt_OTHER", "drv_001")).rejects.toThrow(
      "MOTUS_UNAUTHORIZED"
    );
  });
});
