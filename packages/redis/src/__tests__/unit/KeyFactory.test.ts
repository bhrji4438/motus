...
describe("KeyFactory", () => {
  ...
  describe("Cluster hashtag compliance", () => {
    ...
    it("uses default prefix 'vectro' for global session expiry keys", () => {
      KeyFactory.prefix = "vectro";
      expect(KeyFactory.sessionExpiryZset()).toBe("vectro:sessions:expiry");
    });

    it("respects custom prefix when changed", () => {
      KeyFactory.prefix = "custom";
      expect(KeyFactory.sessionExpiryZset()).toBe("custom:sessions:expiry");
      KeyFactory.prefix = "vectro"; // Reset back to default
    });
  ...
  describe("Lock keys", () => {
    it("generates sessionLock", () => {
      expect(KeyFactory.sessionLock(sessionId)).toBe(
    ...
    it("generates presenceScanLock", () => {
      expect(KeyFactory.presenceScanLock(tenantId)).toBe(
        `lock:presence:stale_scan:${tenantId}`
      );
    });
