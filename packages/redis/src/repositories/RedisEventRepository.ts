...
export class RedisEventRepository {
   ...
   */
  async appendEvent(tenantId: TenantId, sessionId: SessionId, event: SessionEvent): Promise<string> {
    ...
    return withObservability(this.obs, 'RedisEventRepository.appendEvent', async () => {
      ...
      for (let attempt = 1; attempt <= this.streams.eventAppendMaxRetries; attempt++) {
        try {
          ...
          const streamId = await (this.client as any).xadd(key, '*', ...args) as string;
      ...
      this.obs.logger.error(`Failed to append event after ${this.streams.eventAppendMaxRetries} attempts`, {
        tenantId, sessionId, eventId: event.eventId, error: lastError,
      });
      throw lastError;
    });
  }
  ...
  async getEvents(tenantId: TenantId, sessionId: SessionId, options?: GetEventsOptions): Promise<SessionEvent[]> {
    TenantGuard.validate(tenantId);
    TenantGuard.validateSessionId(sessionId);
    const key = KeyFactory.sessionEventStream(tenantId, sessionId);

    return withObservability(this.obs, 'RedisEventRepository.getEvents', async () => {
  ...
  async getEventTimeline(tenantId: TenantId, sessionId: SessionId): Promise<SessionEvent[]> {
    return this.getEvents(tenantId, sessionId);
  }

  /** Returns the N most recent events in reverse chronological order. */
  async getRecentEvents(tenantId: TenantId, sessionId: SessionId, count: number): Promise<SessionEvent[]> {
    TenantGuard.validate(tenantId);
    TenantGuard.validateSessionId(sessionId);
    const key = KeyFactory.sessionEventStream(tenantId, sessionId);

    return withObservability(this.obs, 'RedisEventRepository.getRecentEvents', async () => {
  ...
  async countEvents(tenantId: TenantId, sessionId: SessionId): Promise<number> {
    ...
    TenantGuard.validateSessionId(sessionId);
    const key = KeyFactory.sessionEventStream(tenantId, sessionId);
    return withObservability(this.obs, 'RedisEventRepository.countEvents', async () => {
      return (this.client as any).xlen(key) as Promise<number>;
    });
  }

  /** Trims the event stream to the most recent maxLen entries. */
  async trimEvents(tenantId: TenantId, sessionId: SessionId, maxLen: number, exact = false): Promise<void> {
    TenantGuard.validate(tenantId);
    TenantGuard.validateSessionId(sessionId);
    const key = KeyFactory.sessionEventStream(tenantId, sessionId);
    await withObservability(this.obs, 'RedisEventRepository.trimEvents', async () => {
      if (exact) {
  ...
  async deleteEvents(tenantId: TenantId, sessionId: SessionId): Promise<void> {
    ...
    TenantGuard.validateSessionId(sessionId);
    const key = KeyFactory.sessionEventStream(tenantId, sessionId);
    await withObservability(this.obs, 'RedisEventRepository.deleteEvents', async () => {
