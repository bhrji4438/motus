...
export class RedisTelemetryRepository {
  ...
  async appendTelemetry(
    ...
    return withObservability(
      this.obs,
      "RedisTelemetryRepository.appendTelemetry",
      async () => {
        const args: string[] = [];
        ...
        if (this.streams.telemetryDropOnFailure) {
          try {
            const id = (await (this.client as any).xadd(
              key,
              "*",
              ...args
            )) as string;
  ...
  async getTelemetryPath(
    ...
    return withObservability(
      this.obs,
      "RedisTelemetryRepository.getTelemetryPath",
      async () => {
  ...
  async getRecentTelemetry(
    ...
    return withObservability(
      this.obs,
      "RedisTelemetryRepository.getRecentTelemetry",
      async () => {
  ...
  async countTelemetryPoints(
    ...
    return withObservability(
      this.obs,
      "RedisTelemetryRepository.countTelemetryPoints",
      async () => {
  ...
  async trimTelemetry(
    ...
    await withObservability(
      this.obs,
      "RedisTelemetryRepository.trimTelemetry",
      async () => {
        if (exact) {
  ...
  async deleteTelemetry(
    ...
    await withObservability(
      this.obs,
      "RedisTelemetryRepository.deleteTelemetry",
      async () => {
