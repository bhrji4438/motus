...
import { SessionSerializer, TelemetrySerializer, EventStreamSerializer } from '@/serialization/Serializer.js';
...
export class RedisSessionRepository implements ISessionRepository {
  ...
  async save(session: Session & { requiredVehicleType?: string }): Promise<void> {
    ...
    await withObservability(this.obs, 'RedisSessionRepository.save', async () => {
      const args: string[] = [];
      ...
      if (TERMINAL_STATES.has(session.status as string)) {
        const expiryScore = Date.now() + this.retention.sessionRetentionSeconds * 1000;
        const member = KeyFactory.sessionExpiryMember(session.tenantId, session.id);
  ...
  async get(tenantId: TenantId, sessionId: SessionId): Promise<Session | null> {
    ...
    return withObservability(this.obs, 'RedisSessionRepository.get', async () => {
      ...
      const [hashErr, hashFields] = results[0] as [Error | null, Record<string, string> | null];
      ...
      if (hashFields['tenantId'] && hashFields['tenantId'] !== tenantId) {
        ...
        throw new Error(`MOTUS_UNAUTHORIZED: Cross-tenant session access denied for sessionId ${sessionId}`);
      }

      const session = SessionSerializer.deserialize(hashFields);

      // Hydrate telemetry path from stream
      const [, telemetryEntries] = results[1] as [Error | null, Array<[string, string[]]>];
      ...
      const [, eventEntries] = results[2] as [Error | null, Array<[string, string[]]>];
