...
export class RedisPresenceRepository {
  ...
  async heartbeat(
    ...
    await withObservability(
      this.obs,
      "RedisPresenceRepository.heartbeat",
      async () => {
  ...
  async remove(tenantId: TenantId, driverId: DriverId): Promise<void> {
    ...
    await withObservability(
      this.obs,
      "RedisPresenceRepository.remove",
      async () => {
  ...
  async getStaleDrivers(
    ...
    return withObservability(
      this.obs,
      "RedisPresenceRepository.getStaleDrivers",
      async () => {
  ...
  async getActiveDrivers(tenantId: TenantId): Promise<PresenceEntry[]> {
    ...
    return withObservability(
      this.obs,
      "RedisPresenceRepository.getActiveDrivers",
      async () => {
        const results = (await (this.client as any).zrangebyscore(
          key,
          "-inf",
  ...
  async getHeartbeat(
    ...
    return withObservability(
      this.obs,
      "RedisPresenceRepository.getHeartbeat",
      async () => {
        const score = (await (this.client as any).zscore(key, driverId)) as
          | string
          | null;
