...
export function registerFastifyHooks(fastifyInstance: any): void {
  ...
  fastifyInstance.addHook('onResponse', async (request: any, reply: any) => {
    ...
    if (startTime) {
      ...
      logger.info(`HTTP Request Complete: ${request.method} ${request.url} - Status ${statusCode} - ${durationSec.toFixed(3)}s`);
    }
  ...
  fastifyInstance.addHook('onError', async (request: any, _reply: any, error: Error) => {
    const span = request.otelSpan as Span;
    if (span) {
      span.recordException(error);
      span.setStatus({ code: 2, message: error.message });
    }
    logger.error(`HTTP Request Error: ${request.method} ${request.url}`, error);
  });
}
...
export function expressMiddleware() {
  return (req: any, res: any, next: () => void) => {
    ...
    res.on('finish', () => {
      ...
      logger.info(`HTTP Request Complete: ${req.method} ${req.url} - Status ${statusCode} - ${durationSec.toFixed(3)}s`);
