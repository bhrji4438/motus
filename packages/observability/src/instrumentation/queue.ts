    ...
    return Tracer.runWithSpan(
      ...
      async (span) => {
        ...
        try {
          ...
          this.recordDuration(queueName, jobType, 'publish', startTime, 'success');
          return result;
        } catch (error: any) {
          this.recordDuration(queueName, jobType, 'publish', startTime, 'error');
          ErrorTracker.captureException(error, `Failed to publish job to queue ${queueName}`, {
    ...
    return Tracer.runWithSpan(
      ...
      async (span) => {
        ...
        try {
          ...
          this.recordDuration(queueName, jobType, 'process', startTime, 'success');
          return result;
        } catch (error: any) {
          this.recordDuration(queueName, jobType, 'process', startTime, 'error');
          ErrorTracker.captureException(error, `Failed to process job in queue ${queueName}`, {
            queueSystem: this.queueSystem,
            queueName,
            jobType,
          });
