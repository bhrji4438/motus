...
export type TenantCreatedEvent = EventEnvelope<
  "tenant.created",
  TenantCreatedPayload
>;
export type DriverOnlineEvent = EventEnvelope<
  "driver.online",
  DriverOnlinePayload
>;
export type DriverOfflineEvent = EventEnvelope<
  "driver.offline",
  DriverOfflinePayload
>;
export type DriverPausedEvent = EventEnvelope<
  "driver.paused",
  DriverPausedPayload
>;
export type DriverLocationUpdatedEvent = EventEnvelope<
  "driver.location.updated",
  DriverLocationUpdatedPayload
>;
export type TelemetrySampledEvent = EventEnvelope<
  "telemetry.sampled",
  TelemetrySampledPayload
>;
export type SessionCreatedEvent = EventEnvelope<
  "session.created",
  SessionCreatedPayload
>;
export type SessionSearchingEvent = EventEnvelope<
  "session.searching",
  SessionSearchingPayload
>;
export type SessionAssignedEvent = EventEnvelope<
  "session.assigned",
  SessionAssignedPayload
>;
export type SessionArrivedEvent = EventEnvelope<
  "session.arrived",
  SessionArrivedPayload
>;
export type SessionStartedEvent = EventEnvelope<
  "session.started",
  SessionStartedPayload
>;
export type SessionCompletedEvent = EventEnvelope<
  "session.completed",
  SessionCompletedPayload
>;
export type SessionCancelledEvent = EventEnvelope<
  "session.cancelled",
  SessionCancelledPayload
>;
export type SessionDriverLostEvent = EventEnvelope<
  "session.driver_lost",
  SessionDriverLostPayload
>;
export type DispatchWaveStartedEvent = EventEnvelope<
  "dispatch.wave.started",
  DispatchWaveStartedPayload
>;
export type DispatchWaveCompletedEvent = EventEnvelope<
  "dispatch.wave.completed",
  DispatchWaveCompletedPayload
>;
export type DispatchNoDriverFoundEvent = EventEnvelope<
  "dispatch.no_driver_found",
  DispatchNoDriverFoundPayload
>;
...
export const EVENT_GOVERNANCE_REGISTRY: {
  readonly [K in MotusEvent["eventName"]]: EventGovernanceMetadata;
} = {
  "tenant.created": {
  ...
  "driver.location.updated": {
    ...
    partitionKey: "driverId",
    idempotencyRequirements:
      "Discard out-of-order locations by checking update timestamp sequence.",
    version: "1.0.0",
  },
  "telemetry.sampled": {
    ...
    idempotencyRequirements:
      "Append point to historical route path sequentially.",
    version: "1.0.0",
  },
  "session.created": {
  ...
  "session.arrived": {
    ...
    partitionKey: "sessionId",
    idempotencyRequirements:
      "Trigger notification and update state machine context.",
    version: "1.0.0",
  },
  "session.started": {
    producer: "SessionService",
