...
describe('Phase 3 Wave Distribution: Fanout & Assignment', () => {
  ...
  it('should run fanout next wave and acquire candidate locks', async () => {
    ...
    const sessionMgr = new SessionManager(sessionRepo, lockMgr, eventBus, clock, idGen, driverMgr);
    const matchingEngine = new MatchingEngine(tenantRepo, driverRepo, clock, logger, metrics);
    ...
    await tenantRepo.save({
      id: 'tnt_1',
      name: 'Tenant 1',
      matchingConfig: { strategy: MatchingStrategy.DISTANCE, maxSearchRadius: { value: 15000, unit: 'METERS' }, maxCandidatesPerWave: 3 },
      fanoutConfig: { mode: 'PARALLEL', intervalSeconds: 5 },
      retryPolicy: { maxWaves: 3, waveTimeoutSeconds: 8, reEvaluationDelaySeconds: 10 },
      zones: []
    });
    ...
    await driverRepo.save({
      ...driver,
      location: { latitude: 10, longitude: 10, timestamp: clock.now().toISOString() }
    });
    ...
    expect(lockMgr.acquireLock).toHaveBeenCalledWith('lock:candidate:drv_1:session:ses_1', 8);
