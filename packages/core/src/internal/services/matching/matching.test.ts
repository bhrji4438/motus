...
describe("Ray-casting algorithm (raycast)", () => {
  ...
  it("should return true for points inside the polygon", () => {
    expect(isPointInPolygon({ latitude: 15, longitude: 15 }, polygon)).toBe(
      true
    );
  });

  it("should return false for points outside the polygon", () => {
    expect(isPointInPolygon({ latitude: 5, longitude: 5 }, polygon)).toBe(
      false
    );
    expect(isPointInPolygon({ latitude: 25, longitude: 25 }, polygon)).toBe(
      false
    );
  });
  ...
  it("should return false if polygon has less than 3 points", () => {
    ...
    expect(
      isPointInPolygon({ latitude: 15, longitude: 15 }, [
        { latitude: 10, longitude: 10 },
      ])
    ).toBe(false);
  });
...
describe("MatchingEngine", () => {
  ...
  it("should return empty array and log error if tenant is not found", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining("not found during candidate matching.")
    );
  });
  ...
  it("should rank by ETA strategy if selected and provider is available", async () => {
    ...
    etaProvider.calculateEta.mockImplementation(
      async (origin: any, _dest: any) => {
        if (origin.latitude === 15.1)
          return { durationSeconds: 600, distanceMeters: 5000 };
  ...
  it("should fallback to distance ranking if ETA calculation times out (> 100ms)", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining("ETA calculation fallback to distance")
    );
  });
  ...
  it("should catch error and return empty array if findCandidates crashes internally", async () => {
    ...
    expect(candidates).toEqual([]);
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining("Error in MatchingEngine"),
      expect.any(Error)
    );
  });
});
