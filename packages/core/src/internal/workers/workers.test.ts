...
describe("Workers", () => {
  ...
  describe("FanoutTimeoutWorker", () => {
    ...
    it("should transition wave to EXPIRED if timeout duration has passed", async () => {
      ...
      expect(metrics.incrementAssignmentTimeout).toHaveBeenCalledWith(
        "tnt_123"
      );
      expect(fanoutEngine.releaseWaveLocks).toHaveBeenCalledWith("ses_abc", [
        "drv_1",
      ]);
  ...
  describe("DriverStaleDetector", () => {
    it("should detect stale drivers and invoke driverManager.setDriverStale", async () => {
      ...
      expect(driverMgr.setDriverStale).not.toHaveBeenCalledWith(
        "tnt_123",
        "drv_2"
      );
  ...
  describe("DriverLostMonitor", () => {
    ...
    it("should reassign session when recovery timeout is processed", async () => {
      ...
      expect(sessionMgr.reassignSession).toHaveBeenCalledWith(
        expect.objectContaining({
          tenantId: "tnt_123",
          sessionId: "ses_abc",
        })
