...
describe('Phase 1 Foundations: Entities', () => {
  it('should initialize TenantEntity with properties', () => {
    const tenant = new TenantEntity(
      'tnt_123',
      'Test Tenant',
      { strategy: 'DISTANCE', maxSearchRadius: { value: 5000, unit: 'METERS' }, maxCandidatesPerWave: 3 } as any,
      { mode: 'PARALLEL', intervalSeconds: 5 } as any,
      { maxWaves: 3, waveTimeoutSeconds: 8, reEvaluationDelaySeconds: 10 } as any
    );
  ...
  it('should initialize DriverEntity with properties', () => {
    const driver = new DriverEntity(
      ...
      'drv_456',
      DriverStatus.ONLINE,
      { latitude: 12.9716, longitude: 77.5946, timestamp: '2026-06-11T12:00:00Z' },
      0,
      1,
  ...
  it('should initialize SessionEntity and SessionReportEntity', () => {
    const session = new SessionEntity(
      ...
      { latitude: 12.9716, longitude: 77.5946, timestamp: '2026-06-11T12:00:00Z' },
      { latitude: 12.9718, longitude: 77.5948, timestamp: '2026-06-11T12:00:00Z' }
    );
    ...
    const report = new SessionReportEntity(
      ...
      { estimatedDuration: { value: 12, unit: 'MINUTES' }, targetTime: '2026-06-11T12:12:00Z' }
    );
...
describe('Phase 1 Foundations: Error Subsystem', () => {
  it('should create custom MotusCoreError and respect factories', () => {
    ...
    const transitionErr = ErrorFactory.invalidTransition('CREATED', 'COMPLETED', 'Test cause');
...
describe('Phase 1 Foundations: State Machine Engine', () => {
  ...
  it('should validate driver transitions correctly', () => {
    ...
    expect(() =>
      manager.validateDriverTransition(DriverStatus.OFFLINE, DriverStatus.ONLINE, { currentLoad: 0, capacity: 1 })
    ).not.toThrow();

    // Invalid: Offline -> Busy
    expect(() =>
      manager.validateDriverTransition(DriverStatus.OFFLINE, DriverStatus.BUSY, { currentLoad: 0, capacity: 1 })
    ).toThrow();

    // Guard: Online -> Paused when load is > 0
    expect(() =>
      manager.validateDriverTransition(DriverStatus.ONLINE, DriverStatus.PAUSED, { currentLoad: 1, capacity: 2 })
    ).toThrow();

    // Guard: Online -> Paused when load is 0
    expect(() =>
      manager.validateDriverTransition(DriverStatus.ONLINE, DriverStatus.PAUSED, { currentLoad: 0, capacity: 2 })
    ).not.toThrow();
  });
  ...
  it('should validate session transitions correctly', () => {
    ...
    expect(() =>
      manager.validateSessionTransition(SessionState.CREATED, SessionState.SEARCHING)
    ).not.toThrow();

    // Invalid: Created -> Completed
    expect(() =>
      manager.validateSessionTransition(SessionState.CREATED, SessionState.COMPLETED)
    ).toThrow();

    // Guard: Terminal state is immutable
    expect(() =>
      manager.validateSessionTransition(SessionState.COMPLETED, SessionState.SEARCHING)
    ).toThrow();

    // Guard: Driver lost recovery restoring previous state
    expect(() =>
      manager.validateSessionTransition(SessionState.DRIVER_LOST, SessionState.IN_PROGRESS, {
