...
export class EventValidator {
  ...
  public validate(event: MotusEvent): void {
    ...
    if (
      !event.timestamp ||
      !this.metadataVal.validateTimestamp(event.timestamp)
    ) {
      throw ErrorFactory.invalidArgument(
        "timestamp",
    ...
    if (!registry) {
      throw ErrorFactory.invalidArgument(
        "eventName",
        `Event name '${event.eventName}' is not registered in the system.`
      );
    }

    // 3. Producer Verification
    if (
      !this.governanceVal.isAuthorizedProducer(
        event.eventName,
        event.governance.producer
      )
    ) {
      throw ErrorFactory.invalidArgument(
        "governance.producer",
    ...
    if (
      !this.versionVal.isCompatible(event.governance.version, registry.version)
    ) {
      throw ErrorFactory.invalidArgument(
        "governance.version",
