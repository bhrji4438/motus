...
describe("EventMetadata", () => {
  ...
  it("should validate UUIDv4 event IDs correctly", () => {
    expect(
      metadata.validateEventId("a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d")
    ).toBe(true);
...
describe("EventGovernance", () => {
  ...
  it("should verify authorized producers", () => {
    expect(
      governance.isAuthorizedProducer("tenant.created", "TenantService")
    ).toBe(true);
    expect(
      governance.isAuthorizedProducer("tenant.created", "InvalidProducer")
    ).toBe(false);
    expect(
      governance.isAuthorizedProducer("invalid.event" as any, "AnyProducer")
    ).toBe(false);
  });
...
describe("EventValidator", () => {
  ...
  it("should throw error for invalid eventId", () => {
    ...
    expect(() => validator.validate(event)).toThrow(
      "Event ID must be a valid UUIDv4."
    );
  });
  ...
  it("should throw error for invalid tenantId prefix", () => {
    ...
    expect(() => validator.validate(event)).toThrow(
      "Tenant ID must conform to prefix format (tnt_)."
    );
  });
  ...
  it("should throw error for unregistered eventName", () => {
    const event = {
      ...getValidEvent(),
      eventName: "non.existent.event" as any,
    };
    expect(() => validator.validate(event)).toThrow(
  ...
  it("should throw error for incompatible versions", () => {
    ...
    expect(() => validator.validate(event)).toThrow(
      "is incompatible with registered version"
    );
  });
  ...
  it("should throw error for missing partition key", () => {
    ...
    expect(() => validator.validate(event)).toThrow(
      "Event payload is missing required partition key field"
    );
  });
});
...
describe("EventDispatcher", () => {
  ...
  it("should validate and dispatch events to registered listeners", async () => {
    ...
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({ eventName: "driver.online" })
    );
  });
