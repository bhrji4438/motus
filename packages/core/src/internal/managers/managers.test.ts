...
describe("Domain Managers Suite", () => {
  ...
  beforeEach(() => {
    ...
    etaProvider = {
      calculateEta: vi
        .fn()
        .mockResolvedValue({ durationSeconds: 150, distanceMeters: 800 }),
    };
  ...
  describe("DriverManager", () => {
    ...
    it("should throw on update driver not found", async () => {
      const mgr = new DriverManager(driverRepo, eventBus, clock, idGen);
      await expect(
        mgr.updateDriver({ tenantId: "tnt_1", driverId: "drv_missing" })
      ).rejects.toThrow();
    });

    it("should bind and unbind driver load and transition state", async () => {
      const mgr = new DriverManager(driverRepo, eventBus, clock, idGen);
      await mgr.registerDriver({
        tenantId: "tnt_1",
        driverId: "drv_1",
        capacity: 1,
        vehicleType: "CAR",
  ...
  describe("SessionManager", () => {
    ...
    it("should create session and handle cancellation, completion, and reassignment", async () => {
      ...
      // Cancel session
      const sCancel = await sessionMgr.cancelSession({
        tenantId: "tnt_1",
        sessionId: "ses_1",
        idempotencyKey: "i_cancel",
      });
      expect(sCancel.status).toBe(SessionState.CANCELLED);

      // Completing a cancelled session should fail
      await expect(
        sessionMgr.completeSession({
          tenantId: "tnt_1",
          sessionId: "ses_1",
          idempotencyKey: "i_comp",
        })
      ).rejects.toThrow();
      ...
      const sCompleted = await sessionMgr.completeSession({
        tenantId: "tnt_1",
        sessionId: "ses_2",
        idempotencyKey: "i_comp2",
      });
      ...
      const sReassigned = await sessionMgr.reassignSession({
        tenantId: "tnt_1",
        sessionId: "ses_3",
        reason: "reassigning",
        idempotencyKey: "i_reassign",
      });
    ...
    it("should throw if locks cannot be acquired or session not found", async () => {
      ...
      await expect(
        sessionMgr.startMatching("tnt_1", "ses_missing")
      ).rejects.toThrow();
      await expect(
  ...
  describe("AssignmentManager", () => {
    ...
    it("should accept a session offer successfully and release candidate locks", async () => {
      ...
      expect(
        updated.waves[0].assignments.find((a: any) => a.driverId === "drv_1")
          .status
      ).toBe("ACCEPTED");
      expect(
        updated.waves[0].assignments.find((a: any) => a.driverId === "drv_2")
          .status
      ).toBe("EXPIRED");

      expect(fanoutEngine.releaseWaveLocks).toHaveBeenCalledWith("ses_1", [
        "drv_2",
      ]);
    });
    ...
    it("should throw if lock acquisition fails during acceptance or rejection", async () => {
      lockMgr.acquireLock.mockResolvedValue(false);
      await expect(
        assignmentMgr.acceptSessionOffer("tnt_1", "drv_1", "ses_1", 1)
      ).rejects.toThrow();
      await expect(
        assignmentMgr.rejectSessionOffer("tnt_1", "drv_1", "ses_1", 1)
      ).rejects.toThrow();
    });

    it("should throw if session not found during offer acceptance or rejection", async () => {
      await expect(
        assignmentMgr.acceptSessionOffer("tnt_1", "drv_1", "ses_nonexistent", 1)
      ).rejects.toThrow();
      await expect(
        assignmentMgr.rejectSessionOffer("tnt_1", "drv_1", "ses_nonexistent", 1)
      ).rejects.toThrow();
    });
    ...
    it("should throw if driver lock fails during offer acceptance", async () => {
      ...
      await expect(
        assignmentMgr.acceptSessionOffer("tnt_1", "drv_1", "ses_1", 2)
      ).rejects.toThrow("Failed to acquire lock for key: lock:driver:drv_1");
    });
  });
  ...
  describe("TrackingManager", () => {
    ...
    beforeEach(async () => {
      driverMgr = new DriverManager(driverRepo, eventBus, clock, idGen);
      trackingMgr = new TrackingManager(
        driverRepo,
        eventBus,
        clock,
        idGen,
    ...
    it("should ingest location updates with speed/bearing range validation", async () => {
      await expect(
        trackingMgr.updateDriverLocation({
          tenantId: "tnt_1",
          driverId: "drv_1",
