import { UpdateDriverLocationCommand } from "@motus/types";
import {
  IDriverRepository,
  IEventBus,
  IClock,
...
export class TrackingManager {
  ...
  private readonly lastUpdates = new Map<
    string,
    { timestamp: number; latitude: number; longitude: number; speed: number }
  >();
  ...
  public async updateDriverLocation(
    ...
    if (command.latitude < -90.0 || command.latitude > 90.0) {
      throw ErrorFactory.invalidArgument(
        "latitude",
        `Latitude must be in range [-90.0, 90.0]. Provided: ${command.latitude}`
      );
    }
    if (command.longitude < -180.0 || command.longitude > 180.0) {
      throw ErrorFactory.invalidArgument(
        "longitude",
        `Longitude must be in range [-180.0, 180.0]. Provided: ${command.longitude}`
      );
    }
    if (
      command.bearing !== undefined &&
      (command.bearing < 0.0 || command.bearing > 360.0)
    ) {
      throw ErrorFactory.invalidArgument(
        "bearing",
        `Bearing must be in range [0.0, 360.0]. Provided: ${command.bearing}`
      );
    }
    if (
      command.speed !== undefined &&
      (command.speed < 0.0 || command.speed > 100.0)
    ) {
      throw ErrorFactory.invalidArgument(
        "speed",
        `Speed must be in range [0.0, 100.0]. Provided: ${command.speed}`
      );
    }
    ...
    if (packetTime > serverNow + 500) {
      throw ErrorFactory.invalidArgument(
        "timestamp",
        `Timestamp is too far in the future. Packet: ${
          command.timestamp
        }, Server: ${this.clock.now().toISOString()}`
      );
    }
    ...
    if (cached) {
      ...
      const isStationary =
        command.speed === 0 ||
        (command.speed === undefined && cached.speed === 0);
