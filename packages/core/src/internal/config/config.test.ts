...
import { DEFAULT_MATCHING_CONFIG, DEFAULT_FANOUT_CONFIG, DEFAULT_TELEMETRY_CONFIG } from '@/internal/config/DefaultConfiguration.js';
...
describe('ConfigurationValidator', () => {
  ...
  describe('validateServer', () => {
    ...
    it('should fail on invalid port', () => {
      expect(() => validator.validateServer({ ...validConfig, port: 0 })).toThrow('Port must be in range');
      expect(() => validator.validateServer({ ...validConfig, port: 70000 })).toThrow('Port must be in range');
    });

    it('should fail on empty host', () => {
      expect(() => validator.validateServer({ ...validConfig, host: '' })).toThrow('Host bind address must not be empty.');
    });

    it('should fail on empty jwtSecret', () => {
      expect(() => validator.validateServer({ ...validConfig, jwtSecret: '' })).toThrow('JWT Secret must not be empty.');
      expect(() => validator.validateServer({ ...validConfig, jwtSecret: '   ' })).toThrow('JWT Secret must not be empty.');
    });

    it('should fail on invalid logLevel', () => {
      expect(() => validator.validateServer({ ...validConfig, logLevel: 'invalid' as any })).toThrow('Log level must be one of:');
    });
  });
  ...
  describe('validateTelemetry', () => {
    ...
    it('should fail on invalid distance', () => {
      expect(() => validator.validateTelemetry({ ...validConfig, sampleDistanceMeters: 0 })).toThrow('Sample distance meters must be positive.');
    });

    it('should fail on invalid interval', () => {
      expect(() => validator.validateTelemetry({ ...validConfig, sampleIntervalSeconds: -5 })).toThrow('Sample interval seconds must be positive.');
    });

    it('should fail on invalid TTL', () => {
      expect(() => validator.validateTelemetry({ ...validConfig, streamTtlSeconds: 0 })).toThrow('Stream TTL seconds must be positive.');
    });
  });
  ...
  describe('validateMatching', () => {
    ...
    it('should fail on invalid defaultStrategy', () => {
      expect(() => validator.validateMatching({ ...validConfig, defaultStrategy: 'invalid' as any })).toThrow('Matching default strategy must be one of:');
    });

    it('should fail on invalid initialRadiusMeters', () => {
      expect(() => validator.validateMatching({ ...validConfig, initialRadiusMeters: 0 })).toThrow('Initial radius meters must be in range');
      expect(() => validator.validateMatching({ ...validConfig, initialRadiusMeters: 150000 })).toThrow('Initial radius meters must be in range');
    });

    it('should fail on invalid maxRadiusMeters', () => {
      expect(() => validator.validateMatching({ ...validConfig, maxRadiusMeters: 0 })).toThrow('Max radius meters must be in range');
    });

    it('should fail if initialRadiusMeters exceeds maxRadiusMeters', () => {
      expect(() => validator.validateMatching({ ...validConfig, initialRadiusMeters: 12000, maxRadiusMeters: 10000 })).toThrow('must not exceed maximum search radius');
    });
  });
  ...
  describe('validateFanout', () => {
    ...
    it('should fail on invalid waveSize', () => {
      expect(() => validator.validateFanout({ ...validConfig, waveSize: 0 })).toThrow('Fanout waveSize must be in range [1, 20].');
      expect(() => validator.validateFanout({ ...validConfig, waveSize: 25 })).toThrow('Fanout waveSize must be in range [1, 20].');
    });

    it('should fail on invalid waveTimeoutSeconds', () => {
      expect(() => validator.validateFanout({ ...validConfig, waveTimeoutSeconds: 4 })).toThrow('Fanout waveTimeoutSeconds must be in range [5, 60].');
      expect(() => validator.validateFanout({ ...validConfig, waveTimeoutSeconds: 61 })).toThrow('Fanout waveTimeoutSeconds must be in range [5, 60].');
    });
  });
});
...
describe('ConfigurationManager', () => {
  ...
  it('should apply tenant overrides when configProvider returns overrides', async () => {
    ...
    const featureEnabled = await manager.isFeatureEnabled('tnt_1', 'new-routing');
