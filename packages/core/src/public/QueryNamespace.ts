...
export class QueryNamespace implements IQueryNamespace {
  ...
  public async getSession(tenantId: TenantId, sessionId: SessionId): Promise<SessionResult> {
  ...
  public async getSessionEvents(tenantId: TenantId, sessionId: SessionId): Promise<readonly EventResult[]> {
    const session = await this.sessionMgr.getSession(tenantId, sessionId);
  ...
  public async getSessionReport(tenantId: TenantId, sessionId: SessionId): Promise<SessionReportResult> {
    if (this.reportGen) {
