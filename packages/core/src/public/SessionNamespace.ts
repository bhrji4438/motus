...
export class SessionNamespace implements ISessionNamespace {
  ...
  public async createSession(command: CreateSessionCommand): Promise<SessionResult> {
    const session = await this.sessionMgr.createSession(command);
    return this.mapSessionToResult(session);
  }

  public async cancelSession(command: CancelSessionCommand): Promise<SessionResult> {
    const session = await this.sessionMgr.cancelSession(command);
    return this.mapSessionToResult(session);
  }

  public async completeSession(command: CompleteSessionCommand): Promise<SessionResult> {
    const session = await this.sessionMgr.completeSession(command);
    return this.mapSessionToResult(session);
  }

  public async reassignSession(command: ReassignSessionCommand): Promise<SessionResult> {
  ...
  private mapSessionToResult(session: any): SessionResult {
    ...
    const res: any = {
      ...
      pickup: { latitude: session.pickupPoint.latitude, longitude: session.pickupPoint.longitude },
      destination: { latitude: session.destinationPoint.latitude, longitude: session.destinationPoint.longitude },
      createdAt: session.pickupPoint.timestamp || nowStr,
      updatedAt: nowStr
    };
    if (session.assignedDriverId !== undefined && session.assignedDriverId !== null) {
