...
describe("Public Facades", () => {
  ...
  describe("Motus (Main Facade)", () => {
    it("should initialize and register dependencies correctly", () => {
      ...
      expect(eventDispatcher.off).toHaveBeenCalledWith(
        "driver.online",
        handler
      );

      sdk.events.once("driver.online", handler);
      expect(eventDispatcher.once).toHaveBeenCalledWith(
        "driver.online",
        handler
      );
    });
    ...
    it("should ignore driver namespace actions if tracking or assignment managers are not configured", async () => {
      ...
      await expect(
        facade.acceptSessionOffer("tnt_1", "drv_1", "ses_1", 1)
      ).resolves.not.toThrow();
      await expect(
        facade.rejectSessionOffer("tnt_1", "drv_1", "ses_1", 1)
      ).resolves.not.toThrow();
    });
  });
  ...
  describe("DriverNamespace", () => {
    it("should register, update, get, online, offline, paused drivers", async () => {
      ...
      expect(regResult.lastLocation).toEqual({
        latitude: 12.34,
        longitude: 56.78,
      });
    ...
    it("should delegate updateLocation, acceptOffer, rejectOffer to corresponding managers", async () => {
      ...
      const assignmentMgr = {
        acceptSessionOffer: vi.fn(),
        rejectSessionOffer: vi.fn(),
      };

      const facade = new DriverNamespace(driverMgr);
      facade.setDependencies(trackingMgr, assignmentMgr);

      await facade.updateDriverLocation({
        tenantId: "tnt_1",
        driverId: "drv_1",
        latitude: 1,
        longitude: 1,
      ...
      expect(assignmentMgr.acceptSessionOffer).toHaveBeenCalledWith(
        "tnt_1",
        "drv_1",
        "ses_1",
        1
      ...
      expect(assignmentMgr.rejectSessionOffer).toHaveBeenCalledWith(
        "tnt_1",
        "drv_1",
        "ses_1",
        1
    ...
    it("should create, cancel, complete, reassign sessions", async () => {
      const mockSession = {
        ...
        pickupPoint: {
          latitude: 1,
          longitude: 1,
          timestamp: "2026-06-11T13:59:00Z",
        },
      ...
      sessionMgr.cancelSession.mockResolvedValue({
        ...mockSession,
        status: SessionState.CANCELLED,
      });
      sessionMgr.completeSession.mockResolvedValue({
        ...mockSession,
      ...
      const res1 = await facade.createSession({
        tenantId: "tnt_1",
        sessionId: "ses_1",
      ...
      const res2 = await facade.cancelSession({
        tenantId: "tnt_1",
        sessionId: "ses_1",
        idempotencyKey: "i",
      });
      ...
      const res3 = await facade.completeSession({
        tenantId: "tnt_1",
        sessionId: "ses_1",
        idempotencyKey: "i",
      });
      ...
      const res4 = await facade.reassignSession({
        tenantId: "tnt_1",
        sessionId: "ses_1",
        reason: "r",
        idempotencyKey: "i",
  ...
  describe("QueryNamespace", () => {
    it("should get session, get session events, get session report", async () => {
      ...
      await expect(facade.getSessionReport("tnt_1", "ses_1")).rejects.toThrow(
