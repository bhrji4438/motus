...
export class ConfigurationManager {
  ...
  public async getMatchingConfig(tenantId: TenantId): Promise<MatchingConfig> {
    const defaultStrategy =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "matching.defaultStrategy"
      )) || DEFAULT_MATCHING_CONFIG.defaultStrategy;
    const initialRadiusMeters =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "matching.initialRadiusMeters"
      )) || DEFAULT_MATCHING_CONFIG.initialRadiusMeters;
    const maxRadiusMeters =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "matching.maxRadiusMeters"
      )) || DEFAULT_MATCHING_CONFIG.maxRadiusMeters;
  ...
  public async getFanoutConfig(tenantId: TenantId): Promise<FanoutConfig> {
    const waveSize =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "fanout.waveSize"
      )) || DEFAULT_FANOUT_CONFIG.waveSize;
    const waveTimeoutSeconds =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "fanout.waveTimeoutSeconds"
      )) || DEFAULT_FANOUT_CONFIG.waveTimeoutSeconds;
  ...
  public async getTelemetryConfig(
    tenantId: TenantId
  ): Promise<TelemetryConfig> {
    const sampleDistanceMeters =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "telemetry.sampleDistanceMeters"
      )) || DEFAULT_TELEMETRY_CONFIG.sampleDistanceMeters;
    const sampleIntervalSeconds =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "telemetry.sampleIntervalSeconds"
      )) || DEFAULT_TELEMETRY_CONFIG.sampleIntervalSeconds;
    const streamTtlSeconds =
      (await this.configProvider.getTenantOverride(
        tenantId,
        "telemetry.streamTtlSeconds"
      )) || DEFAULT_TELEMETRY_CONFIG.streamTtlSeconds;
  ...
  public async isFeatureEnabled(
    ...
    const value = await this.configProvider.getTenantOverride(
      tenantId,
      `features.${flag}`
    );
