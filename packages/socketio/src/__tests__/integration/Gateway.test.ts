...
describe('Gateway Integration Tests', () => {
  ...
  const connectClient = (token: string, tenantId = 'tnt_1'): Promise<ClientSocket> => {
  ...
  it('implements DriverGateway inbound actions', async () => {
    ...
    expect(mockDriverNamespace.setDriverOnline).toHaveBeenCalledWith('tnt_1', 'drv_1');
    ...
    await new Promise((r) => setTimeout(r, 100));
    expect(mockDriverNamespace.acceptSessionOffer).toHaveBeenCalledWith('tnt_1', 'drv_1', 'ses_123', 1);
  ...
  it('applies tracking optimizations (decimation and rate-limiting)', async () => {
    ...
    const customTrackingGateway = new (server.trackingGateway.constructor as any)(
      server.transport,
      server.roomManager,
      server.eventRouter,
      1.0, // 1m decimation
    ...
    const success4 = customTrackingGateway.broadcastTrackingUpdate('ses_opt', {
      location: { latitude: 37.7760, longitude: -122.4200 },
      timestamp: new Date().toISOString(),
    });
