...
import { createInvalidArgumentError, createUnauthorizedError } from '@/errors/errors.js';
...
export class EventRouter {
   ...
   */
  public async handleClientEvent(socketId: string, event: string, payload: any): Promise<void> {
    const connection = this.connectionRegistry.getConnection(socketId);
    if (!connection) {
      this.metrics.logger.error(`Connection missing for socket during event routing: ${event}`, { socketId });
    ...
    try {
      switch (event) {
        ...
        case 'subscription:resume':
          ...
          break;
        default:
          throw createInvalidArgumentError(`Unknown or unsupported client event: ${event}`);
      }
    } catch (err) {
      this.metrics.logger.error(`Validation or routing failure for event ${event}`, {
        socketId,
      ...
      const clientError = err instanceof Error && 'toMotusError' in err
      ...
      this.metrics.metrics.recordSocketError(connection.tenantId, clientError.code);
    }
  }

  // ─── Event Handlers ────────────────────────────────────────────────────────

  private async handleDriverPresence(connection: any, payload: any): Promise<void> {
    ...
    this.metrics.logger.info(`Setting driver presence`, { driverId, status: payload.status });
    ...
    switch (payload.status) {
      ...
      case 'PAUSED':
        ...
        break;
      default:
        throw createInvalidArgumentError(`Invalid presence status: ${payload.status}`);
    }
  }

  private async handleDriverLocation(connection: any, payload: any): Promise<void> {
    const { driverId, tenantId } = connection;
    if (!driverId) {
      throw createUnauthorizedError('Only drivers can stream locations');
    }

    if (!payload || !payload.location || typeof payload.location.latitude !== 'number' || typeof payload.location.longitude !== 'number') {
  ...
  private async handleAssignmentAccept(connection: any, payload: any): Promise<void> {
    const { driverId, tenantId } = connection;
    if (!driverId) {
      throw createUnauthorizedError('Only drivers can accept assignments');
    }

    if (!payload || !payload.sessionId || typeof payload.waveNumber !== 'number') {
      throw createInvalidArgumentError('Missing sessionId or waveNumber');
    }

    await this.driverNamespace.acceptSessionOffer(tenantId, driverId, payload.sessionId, payload.waveNumber);
  }

  private async handleAssignmentReject(connection: any, payload: any): Promise<void> {
    const { driverId, tenantId } = connection;
    if (!driverId) {
    ...
    await this.driverNamespace.rejectSessionOffer(tenantId, driverId, payload.sessionId, payload.waveNumber);
  }

  private async handleSessionSubscribe(connection: any, payload: any): Promise<void> {
    ...
    const subscribed = this.subscriptionManager.subscribe(socketId, room, this.maxSubscriptionsPerSocket);
    if (subscribed) {
      await this.roomManager.joinRoom(socket, room, tenantId);
    }
  }

  private async handleSessionUnsubscribe(connection: any, payload: any): Promise<void> {
    const { socketId, tenantId, socket } = connection;
    if (!payload || !payload.sessionId) {
      throw createInvalidArgumentError('Missing required field: sessionId');
    }
  ...
  private async handleTrackingSubscribe(connection: any, payload: any): Promise<void> {
    const { socketId, tenantId, socket } = connection;
    if (!payload || !payload.sessionId) {
      throw createInvalidArgumentError('Missing required field: sessionId');
    }
  ...
  private async handleTrackingUnsubscribe(connection: any, payload: any): Promise<void> {
    const { socketId, tenantId, socket } = connection;
    if (!payload || !payload.sessionId) {
      throw createInvalidArgumentError('Missing required field: sessionId');
    }
  ...
  private async handleSubscriptionResume(connection: any, payload: any): Promise<void> {
    const { socketId, tenantId, socket } = connection;
    ...
    for (const room of payload.rooms) {
      try {
        ...
        const added = this.subscriptionManager.subscribe(socketId, room, this.maxSubscriptionsPerSocket);
        if (added) {
          await this.roomManager.joinRoom(socket, room, tenantId);
          successfullyRecovered.push(room);
        }
      } catch (err) {
        this.metrics.logger.warn(`Failed to recover subscription to room: ${room}`, err);
      }
    }
