...
export class RoomManager {
  ...
  public async joinRoom(socket: any, room: string, tenantId: TenantId): Promise<void> {
    await socket.join(room);
    const roomType = this.getRoomType(room);
    this.metrics.metrics.recordSubscription(tenantId, roomType);
    this.metrics.logger.debug(`Socket joined room`, { socketId: socket.id, room, tenantId });
  }

  public async leaveRoom(socket: any, room: string, tenantId: TenantId): Promise<void> {
    await socket.leave(room);
    const roomType = this.getRoomType(room);
    this.metrics.metrics.recordUnsubscription(tenantId, roomType);
    this.metrics.logger.debug(`Socket left room`, { socketId: socket.id, room, tenantId });
  }

  private getRoomType(room: string): 'tenant' | 'driver' | 'session' | 'tracking' {
