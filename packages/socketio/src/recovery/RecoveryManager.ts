...
export class RecoveryManager {
  ...
  public async recoverSubscriptions(
    ...
    this.metrics.logger.info(`Starting subscription recovery`, { socketId, tenantId, requestedCount: rooms.length });
    ...
    for (const room of rooms) {
      try {
        ...
        const added = this.subscriptionManager.subscribe(socketId, room, maxSubscriptions);
        if (added) {
  ...
  private validateRoomTenantAccess(room: string, tenantId: string): void {
    ...
    if (room.startsWith('tenant:')) {
      ...
      if (roomTenantId !== tenantId) {
        throw createUnauthorizedError(`Cannot join room belonging to another tenant: ${room}`);
      }
    }
