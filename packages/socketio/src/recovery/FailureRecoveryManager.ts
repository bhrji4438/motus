...
export class FailureRecoveryManager {
  ...
  private readonly offlineEventBuffer: { room: string; event: string; payload: any; expiresAt: number }[] = [];
   ...
   */
  public handleDriverDisconnect(tenantId: TenantId, driverId: DriverId, socketId: string): void {
    ...
    const timer = setTimeout(async () => {
      ...
      this.metrics.logger.warn(`Driver grace period expired. Transitioning presence to OFFLINE.`, {
        tenantId,
        driverId,
      });
      ...
      try {
        ...
        this.metrics.logger.error(`Failed to set driver presence offline after disconnect.`, err);
        this.metrics.metrics.recordSocketError(tenantId, 'PRESENCE_TRANSITION_FAILED');
      }
    }, this.disconnectGraceSeconds * 1000);

    this.disconnectTimers.set(key, timer);
    this.metrics.logger.info(`Driver disconnected. Offline grace timer started (${this.disconnectGraceSeconds}s).`, {
      socketId,
      driverId,
    });
  }
   ...
   */
  public handleDriverReconnect(tenantId: TenantId, driverId: DriverId): boolean {
    const key = `${tenantId}:${driverId}`;
    const timer = this.disconnectTimers.get(key);
    if (timer) {
      clearTimeout(timer);
      this.disconnectTimers.delete(key);
      this.metrics.logger.info(`Driver reconnected before grace period expired. Offline timer cancelled.`, {
        driverId,
      });
      return true;
    }
   ...
   */
  public bufferOutgoingEvent(room: string, event: string, payload: any, ttlMs: number = 10000): void {
