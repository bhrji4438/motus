...
export class SessionInspector {
  ...
  constructor() {
    ...
    this.mockSessions.set('S100', {
      ...
      tenantId: 'T1',
      status: 'IN_PROGRESS',
      pickupPoint: { latitude: 37.7749, longitude: -122.4194, timestamp: new Date().toISOString() },
      destinationPoint: { latitude: 37.7891, longitude: -122.4014, timestamp: new Date().toISOString() },
      telemetryPath: [
        { latitude: 37.7749, longitude: -122.4194, timestamp: new Date().toISOString() },
        { latitude: 37.7794, longitude: -122.4132, timestamp: new Date().toISOString() },
        { latitude: 37.7842, longitude: -122.4081, timestamp: new Date().toISOString() },
      ],
      eventTimeline: [
        { eventId: 'ev-1', eventName: 'session.created', timestamp: new Date().toISOString(), payload: {} },
        { eventId: 'ev-2', eventName: 'dispatch.wave.started', timestamp: new Date().toISOString(), payload: { waveNumber: 1 } },
        { eventId: 'ev-3', eventName: 'session.assigned', timestamp: new Date().toISOString(), payload: { assignedDriverId: 'driver-1' } },
      ],
      traces: [
        {
          spanId: 'span-1',
   ...
   */
  public async getSessionDetail(tenantId: string, sessionId: string): Promise<any | null> {
   ...
   */
  public async getSessionTraces(tenantId: string, sessionId: string): Promise<TraceSpanData[]> {
