...
describe("RbacGuard", () => {
  ...
  it("should deny request if permission is missing in role schema", () => {
    ...
    expect(RbacGuard.authorize(dispatcherUser, "session.read", "T1")).toBe(
      true
    );
    expect(RbacGuard.authorize(dispatcherUser, "session.write", "T1")).toBe(
      true
    );
  ...
  it("should bypass tenant restrictions for SUPER_ADMIN role", () => {
    ...
    expect(RbacGuard.authorize(superAdminUser, "session.read", "T2")).toBe(
      true
    );
