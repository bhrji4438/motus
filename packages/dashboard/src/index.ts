...
export const dashboardPlugin: FastifyPluginAsync = async (
  ...
  // REST API Routes
  fastify.get(
    "/api/dashboard/:tenantId/analytics",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
        !RbacGuard.authorize(user, "analytics.read", request.params.tenantId)
      ) {
        return reply
          .status(403)
          .send({ error: "Forbidden. Requires analyst or admin permissions." });
      }
      const summary = await defaultAnalytics.getTenantSummary(
  ...
  fastify.get("/api/dashboard/:tenantId/audit", async (request: any, reply) => {
    ...
    if (
      !user ||
      !RbacGuard.authorize(user, "logs.read", request.params.tenantId)
    ) {
      return reply
        .status(403)
        .send({ error: "Forbidden. Requires log view permissions." });
    }
  ...
  fastify.get(
    "/api/dashboard/:tenantId/audit/export",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
      ...
      reply.header(
        "Content-Disposition",
        `attachment; filename=audit-logs-${request.params.tenantId}.csv`
      );
      return csv;
    }
  );
  ...
  fastify.get(
    "/api/dashboard/:tenantId/sessions",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
        !RbacGuard.authorize(user, "session.read", request.params.tenantId)
      ) {
        return reply.status(403).send({ error: "Forbidden." });
      }
      const status = request.query.status;
      const sessions = await defaultSessionInspector.listSessions(
        request.params.tenantId,
        status
      );
      return sessions;
  ...
  fastify.get(
    "/api/dashboard/:tenantId/sessions/:sessionId",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
      ...
      const detail = await defaultSessionInspector.getSessionDetail(
        request.params.tenantId,
  ...
  fastify.get(
    "/api/dashboard/:tenantId/sessions/:sessionId/traces",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
      ...
      const traces = await defaultSessionInspector.getSessionTraces(
        request.params.tenantId,
        request.params.sessionId
      );
      return traces;
  ...
  fastify.get(
    "/api/dashboard/:tenantId/drivers",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
        !RbacGuard.authorize(user, "tenant.read", request.params.tenantId)
      ) {
        return reply.status(403).send({ error: "Forbidden." });
      }
      const status = request.query.status;
      const drivers = await defaultDispatchMonitor.listDrivers(
        request.params.tenantId,
        status
      );
      return drivers;
  ...
  fastify.get(
    ...
    async (request: any, reply) => {
      ...
      const queues = await defaultQueueMonitor.getTenantQueues(
        request.params.tenantId
      );
      return queues;
    }
  ...
  fastify.get(
    "/api/dashboard/:tenantId/notifications",
    async (request: any, reply) => {
      const user = RbacGuard.authenticateRequest(request.headers);
      if (
        !user ||
      ...
      const logs = await defaultNotificationMonitor.getLogs(
        request.params.tenantId
      );
      return logs;
    }
  ...
  fastify.get(
    "/api/dashboard/:tenantId/realtime/sse",
    async (request: any, reply) => {
